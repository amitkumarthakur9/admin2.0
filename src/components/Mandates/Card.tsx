...
export const Card = ({ data, schema }: { data: MandateDataInterface[], schema: any }) => {
                        ...
                        <View className='flex flex-col w-full'>
                            <View className='flex flex-row items-center w-full justify-between'>
                                <View className='flex flex-row items-center justify-start w-8/12'>
                                    <Pressable onPress={() => router.push(`/clients/${dataItem.id}`)} className='flex flex-row rounded-full bg-[#e60202] mr-2 h-10 w-10 items-center justify-center'>
                                        <Text selectable className='text-white'>{getInitials(dataItem.account.name)}</Text>
                                    </Pressable>
                                    <View className='flex flex-col w-full'>
                                        <View className='flex flex-row items-center text-black font-semibold break-all w-9/12'>
                                            <Text selectable className='text-black font-semibold break-all'>{dataItem.account.name} </Text>

                                        </View>
                                        <View className='flex flex-row items-center mt-1 md:mt-0 lg:mt-0 flex-wrap w-9/12'>
                                            <Text selectable className='text-[#6C6A6A] text-sm'>{dataItem.account.clientId}</Text>
                            ...
                            <View className='flex flex-row items-start justify-center w-full flex-wrap mt-3'>
                                <View className='flex flex-col items-center w-3/12 justify-center'>
                                    <Text selectable className='text-black font-bold text-[13px]'>{dataItem.mandateStatus.name || "-"} </Text>
                                    <Text selectable className='text-slate-600 font-base text-[10px]'>Mandate Status</Text>
                                </View>
                                <View className='flex flex-col items-center w-3/12 justify-center'>
                                    <Text selectable className='text-black font-bold text-[13px]'>{dataItem.amount} </Text>
                                    <Text selectable className='text-slate-600 font-base text-[10px]'>Amount</Text>
                                </View>
                                <View className='flex flex-col items-center w-3/12 justify-center'>
                                    <Text selectable className='text-black font-bold text-[13px]'>{dataItem.bankAccount.branchName} </Text>
                                    <Text selectable className='text-slate-600 font-base text-[10px]'>Bank Branch </Text>
                                </View>
                            ...
                            <View className='flex flex-row items-center w-full mt-3'>
                                ...
                                <TouchableRipple rippleColor={"#a2a2a252"} onPress={() => router.push(`mandates/${dataItem.id}`)} className='w-full py-2 rounded-full border-[0.4px] bg-black'>
