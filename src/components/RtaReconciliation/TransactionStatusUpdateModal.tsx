...
export const TransactionStatusModal = ({
    ...
    const handleSubmit = async () => {
        ...
        const statusValue = statuses.find(
            (status, index) => status.displayString == selectedStatus
        )?.value;

        if (statusValue && id) {
            const response: ApiResponse = await RemoteApi.patch(
                `transaction/reconcile/${id}`,
                {
    ...
    return (
        ...
        >
            <Modal.Content>
                <Modal.CloseButton />
                <Modal.Body>
                    <Text className="mb-12 text-md font-medium">
                            ...
                            {statuses?.map((filter, index) => {
                                return (
                                    <Select.Item
                                        ...
                                        value={filter.displayString}
                                        isDisabled={
                                            filter.displayString == "Registered"
                                        }
                                    />
                                );
                            })}
