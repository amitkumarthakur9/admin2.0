...
const CustomDataTable: React.FC<DataTableProps> = ({ data = [] }) => {
    ...
    const [itemsPerPage, onItemsPerPageChange] = React.useState(page);
        ...
        >
            <DataTable>
                <DataTable.Header className="bg-zinc-200">
                    ...
                    <DataTable.Title style={{ width: 125, flexBasis: "auto" }}>
                        Pan
                    </DataTable.Title>
                    <DataTable.Title style={{ width: 120, flexBasis: "auto" }}>
                        Tax Status
                    </DataTable.Title>
                    <DataTable.Title style={{ width: 20, flexBasis: "auto" }}>
                        Action
                    </DataTable.Title>
                </DataTable.Header>
                ...
                {sortedItems.slice(from, to).map((item, index) => (
                    <DataTable.Row key={index} centered>
                        ...
                        <DataTable.Cell
                            style={{ width: 150, flexBasis: "auto" }}
                        >
                            {item.email}
                        </DataTable.Cell>
                        <DataTable.Cell
                            style={{ width: 125, flexBasis: "auto" }}
                        >
                            {item.pan}{" "}
                            <Text selectable className="text-green-500">
                                KYC
                            </Text>
                        </DataTable.Cell>
                        <DataTable.Cell
