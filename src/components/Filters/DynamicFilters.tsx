...
export const DynamicFilters = ({
    ...
    useEffect(() => {
        if (
            ...
            let removedAllFilters = filterValues.filter(
                (obj) => obj.key !== "all"
            );
    ...
    const handleFilterChange = (key, value, operator) => {
        ...
        const existingIndex = filterValues.findIndex(
    ...
    const determineDisplayValue = () => {
        ...
        filtersSchema?.forEach((obj2) => {
            const existsInArr1 = appliedFilers?.some(
                (obj1) => obj1.key === obj2.key && obj1.key !== "all"
            );
    ...
    const searchPlaceholder = (maxPlaceholderLength = 0) => {
        const keysToInclude = filtersSchema
            ?.filter(
                (obj) =>
                    obj.title !== "All" && obj.operator[0].subKey === "contains"
            )
            .map((obj) => obj.title);
    ...
    return (
        <View className="flex flex-row justify-between items-center mt-5 w-100 flex-wrap">
            <View
                        ...
                        <View className="flex flex-row mb-1 lg:mb-0">
                            <View className="ml-0 lg:ml-1">
                                <Pressable
                                    ...
                                    onPress={() => setFilterOpen(!filterOpen)}
                                    className={
                                        "flex flex-row justify-center items-center border-[1px] rounded px-4 py-3 border-slate-200 " +
                                        (filterOpen ? "bg-zinc-100" : "")
                                    }
                                    accessibilityLabel="More options menu"
                                >
                            ...
                            {schemaResponse?.fastFilter && (
                                <ExtraFilters
                                    filtersSchema={schemaResponse?.fastFilter}
