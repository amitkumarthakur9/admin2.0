...
export const MobileSIPRows = ({ data, schema }: { data: SIPReportItems[], schema: any }) => {
                        ...
                        <View className='flex flex-col w-full'>
                            <View className='flex flex-row items-center w-full justify-between'>
                                <View className='flex flex-row items-center justify-start w-8/12'>
                                    <Pressable onPress={() => router.push(`/clients/${sip.account.id}`)} className='flex flex-row rounded-full bg-[#e60202] mr-2 h-10 w-10 items-center justify-center'>
                                        <Text selectable className='text-white'>{getInitials(sip.account.name)}</Text>
                                    </Pressable>
                                    <View className='flex flex-col w-full'>
                                        <View className='flex flex-row items-center text-black font-semibold break-all w-9/12'>
                                            <Text selectable className='text-black font-semibold break-all'>{sip.account.name}&nbsp;</Text>

                                        </View>
                                        <View className='flex flex-row items-center mt-1 md:mt-0 lg:mt-0 flex-wrap w-9/12'>
                                            <Text selectable className='text-[#6C6A6A] text-sm'>{sip.account.clientId}</Text>
                                ...
                                <View className='flex flex-col items-center justify-center w-4/12'>
                                    <View className='flex flex-row items-center w-full justify-center'>
                                        <Text selectable className='p-1 text-black font-bold text-end text-sm'>{RupeeSymbol + sip.amount}&nbsp;</Text>
                                    ...
                                    <View className='flex flex-row items-center rounded-full w-full justify-center'>
                                        <Text selectable className='p-1 text-black text-end text-xs'>{sip.orderStatus.name}&nbsp;</Text>
                                    </View>
                            ...
                            <View className='flex flex-row items-center w-full justify-between mt-2'>
                                <View className={"flex flex-row " + ("w-full")} >
                                    ...
                                    <View className={'flex flex-col justify-end items-start w-8/12'} >
                                        <Text selectable className='text-black font-semibold break-all text-sm flex-wrap' style={{ textAlign: "left" }}>{sip.mutualfund.name}</Text>
                                        <View className='flex flex-row items-center flex-wrap'>
                                            <Text selectable className='text-black text-xs'>{sip.mutualfund?.mutualfundSubcategory?.name}</Text>
                                        </View>
                                    </View>
                            ...
                            <View className='flex flex-row items-start justify-center w-full flex-wrap mt-3'>
                                <View className='flex flex-col items-center w-4/12 justify-start'>
                                    ...
                                    <Text selectable className='text-slate-600 font-base text-[10px] text-center'>SIP Reg</Text>

                                    <Text selectable className='text-black font-bold text-[10px] text-center w-10/12'>{sip.sipReferenceNumber || "-"} </Text>
                                </View>
                                <View className='flex flex-col items-center w-4/12 justify-start'>
                                    <Text selectable className='text-slate-600 font-base text-[10px] text-center'>Folio</Text>

                                    <Text selectable className='text-black font-bold text-[10px] text-center'>{sip.transactions[0]?.folio?.folioNumber || "-"} </Text>
                                </View>
                                <View className='flex flex-col items-center w-4/12 justify-center'>
                                    <Text selectable className='text-slate-600 font-base text-[10px] text-center'>Start Date</Text>

                                    <Text selectable className='text-black font-bold text-[10px] text-center w-10/12'>{sip.startDate ? moment(new Date(sip.startDate)).format('DD-MM-YYYY hh:mm:ss A') : ""} </Text>
                                </View>
                            </View>
                            ...
                            <View className='flex flex-row items-center w-full mt-3'>
                                ...
                                <TouchableRipple rippleColor={"#a2a2a252"} onPress={() => router.push(`sip-reports/${sip.id}`)} className='w-full py-2 rounded-full border-[0.4px] bg-black'>
