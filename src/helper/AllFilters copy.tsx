...
const FilterForm = ({
    ...
    const removeRepeaterElement = (key) => {
        const updatedRepeaterValue = repeaterValue.filter(
            (ele, index) => ele !== key
        );
        ...
        <View>
            ...
            >
                {filtersSchema.map(
                    (filter, key) =>
                        filter.key !== "all" &&
                        !repeaterValue.includes(filter.key) && (
                            <Select.Item
                                key={filter.key}
                                label={filter.title}
                                value={filter.key}
        ...
        <View className="flex flex-row items-center mb-2 justify-between ">
            <View className="flex flex-row w-3/12 flex-wrap">
                <View className="flex w-11/12">
                    <Text selectable className="break-all" style={{}}>
                        {title}
                    </Text>
                </View>
            </View>
            ...
            <View className="w-1/12 flex flex-row">
                <View>
                    <Pressable
                        onPress={() => removeRepeaterElement(filter.key)}
                    >
                        <Icon
                            name="trash"
                ...
                <View>
                    <Pressable onPress={addRepeaterElement}>
                        <Icon name="plus" size={14} color="red" />
                    </Pressable>
                </View>
            </View>
        </View>
